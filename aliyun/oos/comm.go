...
// GetSimpleBucket creates the test bucket
func GetSimpleBucket(bucketName string) (*oss.Bucket, error) {
	// New client
...
func GetProxyBucket(bucketName string) (*oss.Bucket, error) {
	// New client
	client, err := oss.New(endpoint, accessID, accessKey, oss.Proxy("http://47.241.95.81:8888"))
	if err != nil {
		return nil, err
	}
